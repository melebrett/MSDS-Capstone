...
age_effect <- function(proj_df){
  ...
  current <- predict(mod_ls_current,
                     proj_df %>%
                       mutate(current_age = current_age - year_offset) %>%
                       dplyr::select(age = current_age),
                     re.form = NA)
  future <- predict(mod_ls_current,proj_df %>% dplyr::select(age = future_age), re.form = NA)
...
ls_variation <- data.frame(ranefs)[,c("grp","condsd")]
...
for(fy in 2:3){
  ...
  tmp$sd_latent_skill <- tmp$sd_latent_skill*(.05*(fy-1) + 1) # add noise to ls prediction in each year
